 ...
 * License along with this_ program; if not, write to the Free    *
...
/* define this_ to see how each phase of pattern rejection is performing */
...
compile_for_match()
{
  ...
  for (color=0; color<2; ++color) {
	 ...
	 * color==1. If this_ is not true, we swap all the colors on the
	 * board as we merge.
	 */
	if (this_ != EMPTY && color != 0)
	  this_ = OTHER_COLOR(this_);
	  
	for (shift=0, ii = i-2; ii <= i+1 ; ++ii, shift += 8) {
	  if (ii < 0 || ii >= board_size)
	    continue;
	    
	  /* Add this_ one into all the nearby merged_board[][] elements. */
	  if (j > 1)  merged_board[color][ii][j-2] |= this_ << (shift+0);
	  if (j > 0)  merged_board[color][ii][j-1] |= this_ << (shift+2);
	  merged_board[color][ii][j]               |= this_ << (shift+4);
	  if (j < board_size-1) 
	    merged_board[color][ii][j+1] |= this_ << (shift+6);
	}
      }
    }
 ...
 *
 * There is a copy of this_ table in mkpat.c
 */
 ...
 * autohelper. It is up to the callback to process the other klasses,
	 ...
	 int minwt, struct pattern *pattern) 
{
  ...
  /* and a table for checking klasses quickly
   * klass_mask[status][color] contains the mask to look for in klass.
   * ie. if  pat[r].klass & klass_mask[dragon[x][y].status][p[x][y]]
   * is not zero then we reject it
   * Most elements if klass_mask[] are zero - it is a sparse
   * matrix containing
   ...
   *  klass_mask[DEAD][other]  will contain CLASS_X
   * Then we reject any patterns which have CLASS_X
   * set in the klass bits.
   *
  ...
  static int klass_mask[MAX_DRAGON_STATUS][3];
  
  klass_mask[DEAD][color] = CLASS_O;
  klass_mask[DEAD][other] = CLASS_X;
  klass_mask[ALIVE][color] = CLASS_o;
  klass_mask[ALIVE][other] = CLASS_x;
...
//  //assert(m>=0 && m<board_size && n>=0 && n<board_size);
  ...
  for ( ; pattern->patn; ++pattern) { 
     ...
     *     style fearless is enabled, thus with this_ option, a pattern
     *     must have a maxwt 10 points lower than minwt to be discarded.
     */       
    if ( allpats || (pattern->maxwt >= minwt) ||
	 (pattern->klass & (CLASS_D | CLASS_A)) ||
	 (pattern->klass & (CLASS_B | CLASS_C)) ||
	 (pattern->klass & (CLASS_L)) ||
         (style & STY_FEARLESS && minwt < 75 && pattern->maxwt + 10 >= minwt))
    {
       ...
       * at O, or 'other' for X.
       * Since we require that this_ flag contains 3 for
	...
	/* We first perform the grid check : this_ checks up to 16
	 * elements in one go, and allows us to rapidly reject
	 * patterns which do not match.  While this_ check invokes a
	 * necessary condition, it is not a sufficient test, so more
	 * careful checks are still required, but this_ allows rapid
	...
	for (k = 0; k < pattern->patlen; ++k) { /* match each point */
	  ...
	  /* Work out the position on the board of this_ pattern element. */
...
//	  //assert(x>=0 && x < board_size && y >= 0 && y < board_size);
	  ...
	  /* Check out the klass_X, klass_O, klass_x, klass_o attributes - see
	   * patterns.db and above
	   */
	  if ((pattern->klass & klass_mask[dragon[x][y].status][p[x][y]]) != 0)
	    goto match_failed; 
	...
	/* Make sure the grid optimisation wouldn't have rejected this_ pattern */
