	       ...
	       int tablesize, int num_nodes, int num_results)
{
  ...
  table->hashtable_ = (Hashnode **) malloc(tablesize*sizeof(Hashnode *));
  if (table->hashtable_ == NULL) {
    free(table);
    return 0;
  }
			 ...
			 Hashposition *pos, unsigned long hash)
{
  ...
  node->next = table->hashtable_[bucket];
  table->hashtable_[bucket] = node;
