...
shapes_callback(int m, int n, int color, struct pattern *pattern, int ll)
{
   ...
   * element.
   */
  if ((pattern->klass & (CLASS_A | CLASS_B | CLASS_C | CLASS_D | CLASS_L)) != 0
      || pattern->obonus || pattern->xbonus)
  {
	...
	if (p[x][y]
      ...
      if ((pattern->klass & CLASS_D) && (p[x][y]==color) && (defi==-1))
	if (worm[x][y].attacki != -1) {
	  defi = worm[x][y].origini;
	  defj = worm[x][y].originj;
	}
	
      if ((pattern->klass & CLASS_A) && (p[x][y]==other) && (atti==-1)) {
	...
	if (0)
      ...
      if ((pattern->klass & CLASS_B) && (p[x][y]==other)) {
...
//	  //assert(tval <= pattern->maxwt);  
  ...
  if (pattern->klass & (CLASS_B | CLASS_C)) {
    bcval = max(bval, cval);
   ...
   */
  if ((pattern->klass & CLASS_D) && (defi != -1)) {
   ...
   * recommended move instead of some other stupid one.
   */
  if (pattern->klass & CLASS_A && (atti != -1)) {
