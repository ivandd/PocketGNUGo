...
sgfNewNode()
{
  SGFNodeP res = (struct SGFNode *)xalloc(sizeof(*res));
  return res;
}
...
sgfGetFloatProperty(SGFNodeP n, const UCHAR_ALIAS *name, float *value)
{
  ...
  for (pr = n->prop; pr; pr=pr->next)
    if (pr->name == nam) {
      *value = (float) wcstod((const unsigned short *)pr->value,NULL);
...
{
  if (strlen(name) == 1) {
    SGFPropertyP pr = (struct SGFProperty *)xalloc(sizeof(*pr)+strlen(value));
...
sgfAddPlay(SGFNodeP pr, int who, int movex,int movey)
{
  ...
  if (last->child) {
    ...
    new_->parent=last;
    lastnode=new_;
    sgfAddProperty(new_, (who==BLACK) ? "B":"W", move);
...
sgfBoardText(SGFNodeP pr,int i,int j, const char *text)
{
  ...
  ret=sgfNodeCheck(pr);
  if (!str)
    exit(1); //perror("sgfBoardText: Out of memory!\n");
  else {
    sprintf((char *)str,"%c%c:%s",j+'a',i+'a',text);
    sgfAddProperty((struct SGFNode *)ret,"LB",(const char *)str);
    free(str);
  }
